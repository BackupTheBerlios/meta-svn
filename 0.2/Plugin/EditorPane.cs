...
namespace MetaCommunity.Plugin
{
    ...
    //     - An empty meta file in the same directory as NewFileItems.vsdir. In
    //       our case we chose MyMetaFile.meta. Note this file name appears in Editor.vsdir
    ...
    //          - "MyMetaFile" - This is the base file name. New files will initially
    //             be named as MyMetaFile1.meta, MyMetaFile2.meta... etc.
    ///////////////////////////////////////////////////////////////////////////////
    // Editor.vsdir contents:-
    //    MyMetaFile.meta|{3085E1D6-A938-478e-BE49-3546C09A1AB1}|#106|80|#109|0|401|0|#107
    //
    // The fields in order are as follows:-
    //    - MyMetaFile.meta - our empty meta file
                                ...
                                IVsToolboxUser      //Sends notification about Toolbox items to the owner of these items
    {
        ...
        private const string MyExtension = ".meta";
        ...
        /// </summary>
        /// <param name="serviceProvider">  Service provider that gets passed into the base class constructor.</param>
        /// <param name="package">  Our Package instance.</param>
        public EditorPane(ISysServiceProvider serviceProvider, Plugin package)
        ...
        /// <summary>
        /// Initialization routine for the Editor. Loads the list of properties for the meta document 
        /// which will show up in the properties window 
        /// </summary>
        /// <param name="package"></param>
        private void PrivateInit(Plugin package)
        {
            myPackage = package;
            loading = false;
        ...
        private void setupCommands()
        {
            ...
            if (null != mcs)
            {
                ...
                addCommand(mcs, GuidList.guidPluginCmdSet, (int)PkgCmdIDList.icmdStrike,
        ...
        private void onQueryPasteNextTBXCBItem(object sender, EventArgs e)
        {
            ...
            int itemsAvailable;
            clipboardCycler.AreDataObjectsAvailable((IVsToolboxUser)this, out itemsAvailable);
        ...
        private void onPasteNextTBXCBItem(object sender, EventArgs e)
        {
            ...
            clipboardCycler.GetAndSelectNextDataObject((IVsToolboxUser)this, out pDO);
        ...
        private void onFontSize(object sender, EventArgs e)
        {
            ...
            if (null == args.InValue)
            {
                string currentSize = Convert.ToString(Convert.ToInt32(editorControl.RichTextBoxControl.SelectionFont.Size));
                Marshal.GetNativeVariantForObject(currentSize, args.OutValue);
            }
            else
            {
                string fontSize = (string)args.InValue;
                Font f = new Font(editorControl.RichTextBoxControl.SelectionFont.FontFamily, Convert.ToSingle(fontSize), editorControl.RichTextBoxControl.SelectionFont.Style);
                editorControl.RichTextBoxControl.SelectionFont = f;
        ...
        private void DisplayContextMenuAt(Point point)
        {
            ...
            CommandID contextMenuID = new CommandID(GuidList.guidPluginCmdSet, PkgCmdIDList.IDMX_RTF);
        ...
        /// <summary>
        /// This property gets/sets the selection flags that contain certain information
        ...
        {
            pClassID = GuidList.guidPluginEditorFactory;
            return VSConstants.S_OK;
        }
        ...
        private void OnFileChangeEvent(object sender, System.EventArgs e)
        {
            ...
            if (VsUiShell != null)
            {
                ...
                VsUiShell.ShowMessageBox(0, ref tempGuid, fileName, message, null, 0,
        ...
        private void OnGotFocus(object sender, System.EventArgs e)
        {
            ...
            FileChangeTrigger.Interval = 250;
        ...
        int IVsFindTarget.GetSearchImage(uint grfOptions, IVsTextSpanSet[] ppSpans, out IVsTextImage ppTextImage)
        {
            ...
            if (null != ppSpans && ppSpans.Length > 0)
            {
                ppSpans[0].AttachTextImage(ppTextImage);
            }
        ...
        int IVsTextImage.GetLineLength(int iLine, out int piLength)
        {
            int numberOfLines = 0;
            piLength = 0;
            ((IVsTextImage)this).GetLineSize(out numberOfLines);
        ...
        int IVsTextSpanSet.AttachTextImage(object pText)
        {
            ...
            int lineCount = 0;
            spTextImage.GetLineSize(out lineCount);
            ...
            if (lineCount > 0)
            {
                ...
                spTextImage.GetLineLength(lineCount - 1, out lastLineLength);
            }
